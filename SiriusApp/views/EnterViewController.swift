...
class EnterViewController: UIViewController, UITextFieldDelegate, UIScrollViewDelegate {
    ...
    @objc func rePassButtonAction(sender: UIButton!) {
        
        let svc = TapBarViewController()
        self.present(svc, animated: true, completion: nil)
        /*
        let svc = RePassViewController()
        svc.title = "Забыли пароль"
    ...
    func validation() -> Bool {
        ...
        if ((passTextField.text?.count)!>5) && (phoneData.count>6) {
            return true
        } else {
    ...
    func setupLayout(){
        ...
        passTextField.topAnchor.constraint(equalTo: phoneTextField.bottomAnchor, constant: 40).isActive = true
        ...
        rePassButton.translatesAutoresizingMaskIntoConstraints = false
        scrollView.addSubview(rePassButton)
        rePassButton.centerXAnchor.constraint(equalTo: scrollView.centerXAnchor).isActive = true
        rePassButton.widthAnchor.constraint(equalToConstant: self.view.frame.width-100).isActive = true
        rePassButton.heightAnchor.constraint(equalToConstant: 50).isActive = true
        rePassButton.topAnchor.constraint(equalTo: passTextField.bottomAnchor, constant: 20).isActive = true
        rePassButton.addTarget(self, action: #selector(rePassButtonAction), for: .touchUpInside)
        ...
        nextButton.topAnchor.constraint(equalTo: rePassButton.bottomAnchor, constant: 60).isActive = true
